...
class qvip_memory_message_handler extends QUESTA_MVC::questa_mvc_reporter;
  ...
  extern function void report_message(string category, string fileName, int lineNo,
                                      string objectName, string instanceName, string error_no,
                                      string typ, string mess);
endclass
...
function void qvip_memory_message_handler::report_message(
    string category, string fileName, int lineNo, string objectName, string instanceName,
    string error_no, string typ, string mess);

  case (typ)
    "ERROR": begin
      `uvm_error({"QVIP/", category, "/", error_no}, {mess, "(", objectName, " ", instanceName, ")"
                 })
    end
    "FATAL": begin
      `uvm_fatal({"QVIP/", category, "/", error_no}, {mess, "(", objectName, " ", instanceName, ")"
                 })
    end
    "WARNING": begin
    ...
    "CONTINUE", "MESSAGE", "NOTE": begin
      `uvm_info({"QVIP/", category, "/", error_no}, {mess, "(", objectName, " ", instanceName, ")"},
                UVM_MEDIUM)
    end
  endcase  // case (typ)

...
class qvip_memory_agent #(
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual mgc_ddr)::get(
            null, "UVMF_VIRTUAL_INTERFACES", if_name, vip_config_h.m_bfm
        )) begin
      $stacktrace;
      `uvm_fatal(
          "CFG",
          $sformatf(
              "uvm_config_db #( virtual mgc_ddr )::get cannot find interface bfm resource with interface_name %s",
              if_name))
    end
  endfunction

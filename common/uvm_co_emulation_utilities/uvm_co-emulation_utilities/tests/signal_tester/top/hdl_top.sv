...
module hdl_top ();
  ...
  for (ii = 0; ii < NUM_CLK_RSTS; ii++) begin : clk_rst_block
    clock_bfm #(shared_params_pkg::CLK_INIT_HALF_PERIOD_IN_PS,
                shared_params_pkg::CLK_PHASE_OFFSET_IN_PS) clk_if_h(
        clks[ii]
    );
    sync_reset_bfm #(shared_params_pkg::RST_POLARITY) rst_if_h (
        clks[ii],
        rst_n[ii]
    );
    ...
    initial begin  //tbx vif_binding_block
      ...
      uvm_config_db #(virtual clock_bfm #(shared_params_pkg::CLK_INIT_HALF_PERIOD_IN_PS, shared_params_pkg::CLK_PHASE_OFFSET_IN_PS))::set(
          null, "", $psprintf("clk%0d_if_h", ii), clk_if_h);
      uvm_config_db#(virtual sync_reset_bfm #(shared_params_pkg::RST_POLARITY))::set(
          null, "", $psprintf("rst%0d_if_h", ii), rst_if_h);
      uvm_config_db #(virtual async_reset_bfm #(
                         shared_params_pkg::ASYNC_RST_POLARITY,
                         shared_params_pkg::ASYNC_INIT_IDLE_TIME_IN_PS * (ii+1),
                         shared_params_pkg::ASYNC_RST_ACTIVE_IN_PS * (ii+1)
                                                ))::set(
          null, "", $psprintf("async_rst%0d_if_h", ii), async_rst_if_h);
    end
  end
  ...
  signal_driver_bfm #(
      .SIGNAL_SIZE(shared_params_pkg::SIGNAL_DRIVER_SIZE)
  ) cpu_r1_driver (
      .clock(clks[0]),
      .signals_out(cpu_r1)
  );
  ...
  signal_monitor_bfm #(
      .SIGNAL_SIZE(shared_params_pkg::CLK_COUNTER_SIZE),
      .BUFFER_SIZE(shared_params_pkg::CLK_MONITOR_BUF_SIZE)
  ) clk0_count_monitor (
      .clock(clks[0]),
  ...
  initial begin  //tbx vif_binding_block
    ...
    uvm_config_db #(virtual signal_monitor_bfm #(
                     ...
                     shared_params_pkg::CLK_MONITOR_BUF_SIZE))::set(
        null, "", "clk0_count_monitor", clk0_count_monitor);
    uvm_config_db#(virtual signal_driver_bfm #(shared_params_pkg::SIGNAL_DRIVER_SIZE))::set(
        null, "", "cpu_r1_driver", cpu_r1_driver);
    uvm_config_db#(virtual cpu_pc_monitor_bfm #(`CPU_PC_NUMBER_OF_TRIGGERS))::set(
        null, "", "cpu_pc_monitor", cpu_pc_monitor);
  end

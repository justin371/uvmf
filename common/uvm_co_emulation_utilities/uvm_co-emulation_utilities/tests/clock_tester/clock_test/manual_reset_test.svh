class manual_reset_test extends base_test;
  ...
  task initial_test_manual_ctrl(int num);
    ...
    for (int ii = 0; ii < 30; ii++) test_manual_ctrl(num, 0);
  ...
  task test_manual_ctrl(int num, bit set_mc = 0);
    ...
    `uvm_info(report_id, $sformatf("Waiting for %0d clocks for group %0d to assert_reset",
                                   waittime, num), UVM_MEDIUM)
    clk_ctrl[num].wait_clocks(waittime);
    reset_ctrl[num].assert_reset(`__FILE__, `__LINE__);
    waittime = $urandom_range(50, 100);
    `uvm_info(report_id, $sformatf("Waiting for %0d clocks for group %0d to deassert_reset",
                                   waittime, num), UVM_MEDIUM)
    clk_ctrl[num].wait_clocks(waittime);
  ...
  task test_sync_reset(int num, int idle_cycles = 10, int num_clks_active = 100);
    fork
      ...
      test_sync_reset_thread(num, idle_cycles, num_clks_active);
      test_sync_reset_thread(num, idle_cycles, num_clks_active);
      reset_ctrl[num].toggle_reset(.idle_cycles(idle_cycles), .num_clks_active(num_clks_active),
  ...
  task test_async_reset(int num, int idle_cycles = 10, int num_clks_active = 100);
    ...
    fork
      ...
      case (num)
        0:
        async_reset_ctrl0.toggle_reset(.idle_cycles(idle_cycles), .num_clks_active(num_clks_active),
                                       .file(`__FILE__), .line(`__LINE__));
        1:
        async_reset_ctrl1.toggle_reset(.idle_cycles(idle_cycles), .num_clks_active(num_clks_active),
                                       .file(`__FILE__), .line(`__LINE__));
        2:
        async_reset_ctrl2.toggle_reset(.idle_cycles(idle_cycles), .num_clks_active(num_clks_active),
                                       .file(`__FILE__), .line(`__LINE__));
      endcase
    join

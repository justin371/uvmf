package quirky_regs_pkg;
  ...
  class slave_reg extends uvm_reg;
    ...
    function new(string name = "slave_reg", int has_coverage = UVM_NO_COVERAGE);
  ...
  class mirror_reg_cbs extends uvm_reg_cbs;
    ...
    virtual function void post_predict(input uvm_reg_field fld, input uvm_reg_data_t previous,
      ...
      if (parent == null)
        if (!$cast(parent, fld.get_parent())) `uvm_fatal("mirror_reg_cbs", "parent cast failed")

      if (kind != UVM_PREDICT_WRITE) return;
      if (!(path == UVM_FRONTDOOR || path == UVM_PREDICT)) return;

      if (!parent.slave.get_field_by_name(
              "lower"
          ).predict(
              .value(value), .kind(kind), .path(path), .map(map)
          ))
  ...
  class mirror_reg extends uvm_reg;
    ...
    function new(string name = "mirror_reg", int has_coverage = UVM_NO_COVERAGE);
      super.new(name, 16, has_coverage);
    endfunction : new
  ...
  class apply_holding_reg_cbs extends uvm_reg_cbs;
    ...
    virtual function void post_predict(input uvm_reg_field fld, input uvm_reg_data_t previous,
      ...
      if (!parent.data.predict(.value(temp), .kind(UVM_PREDICT_READ), .path(path), .map(map)))
  ...
  class apply_holding_reg extends uvm_reg;
    ...
    function new(string name = "apply_holding_reg", int has_coverage = UVM_NO_COVERAGE);
      super.new(name, 16, has_coverage);
    endfunction : new
  ...
  class go_bit_reg extends uvm_reg;
    ...
    function new(string name = "go_bit_reg", int unsigned n_bits = 32,

...
class clock_ctrl_base extends uvm_object;
  ...
  virtual task wait_clocks(int unsigned numClocksArg = 1, string file = "", int unsigned line = 0);
    `uvm_error({report_id, "::wait_clocks()"},
                 "Need a clock_ctrl object, not a clock_ctrl_base object")
  endtask : wait_clocks
  ...
  static function void count_stat(input string scope = "", input string file = "",

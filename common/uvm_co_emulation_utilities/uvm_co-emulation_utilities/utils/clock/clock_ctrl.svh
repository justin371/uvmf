class clock_ctrl #(
  ...
  `uvm_object_param_utils(clock_ctrl#(INIT_CLOCK_HALF_PERIOD, PHASE_OFFSET_IN_PS))
  ...
  virtual function void set_bfm(
      virtual clock_bfm #(INIT_CLOCK_HALF_PERIOD, PHASE_OFFSET_IN_PS) cbfm);
    bfm = cbfm;
    bfm.proxy = this;
  endfunction : set_bfm
  ;
  ...
  virtual function void check_bfm();
    if (bfm == null)
      `uvm_fatal(
          report_id,
          "BFM handle is null in clock_ctrl object.  Seg Faults will follow if this is not fixed.")
  ...
  virtual function void update_half_period_in_ps(int unsigned half_period);
    if (!dTimeQueue.isEmpty())
      `uvm_error(report_id,
                 "Changing the clock period while threads are advancing time can cause unpredictible behavior.")
    check_bfm();
  ...
  virtual task wait_clocks(int unsigned numClocksArg = 1, string file = "", int unsigned line = 0);
    ...
    forever begin
      ...
      if (nextInQueue == null || nextInQueue.payload.scheduledClock() >= scheduledClock) break;
      pred = nextInQueue;
    end
  ...
  local function void setClockPeriodInPs(int unsigned clockPeriodInPs);
    ...
    if ($time == 0)
      `uvm_warning(report_id,
                   "You are attempting a time advance at simTime==0. This can result in unpredictable behavior due to initialization races. Please sync on a reset generator before making any calls to the time advancer." )

    if (dClockPeriodInPs == 0) begin
      `uvm_error(
          report_id,
          "Detected parameter CLOCK_PERIOD=0 from HDL side. Be sure first time advance is called at time > 0.")
  ...
  local task advanceNotificationThread();
    ...
    forever begin  // {
      ...
      do begin
        ...
        sync = sync.next();
        old.dequeue();
      end while (sync != null && sync.payload.scheduledClock() == currentClock);

class signal_monitor_proxy #(
    int unsigned SIGNAL_SIZE = 4,
    int unsigned BUFFER_SIZE = 1
) extends uvm_object;
  ...
  virtual task wait_for_signal_pattern(input bit [SIGNAL_SIZE-1:0] signal_pattern,
                                       input bit [7:0] pattern_count, output pattern_found,
                                       output bit [SIGNAL_SIZE-1:0] signals_found);
    ...
    `uvm_info(get_full_name, $psprintf(
              "signal_monitor looking for pattern %x found signal pattern = %x\n",
              signal_pattern,
              signals_found
              ), UVM_LOW);

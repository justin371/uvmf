class async_reset_ctrl #(
  ...
  virtual function void check_bfm();
    if (m_bfm == null)
      `uvm_fatal(report_id,
                 "BFM handle is null in async_reset_ctrl object.  Seg Faults will follow if this is not fixed.")
  endfunction : check_bfm

  virtual function void toggle_reset(int idle_cycles = -1, int num_clks_active = -1,
                                     string file = "", int unsigned line = 0);
    check_bfm();

...
`ifdef USE_VTL
  ...
  vtl_apb_master_module #(SLAVE_COUNT, ADDR_WIDTH, WDATA_WIDTH, RDATA_WIDTH) apb3_master (
  ...
  generate
    if (HAS_MON) begin : monitor_block
      vtl_apb_monitor_module #(ADDR_WIDTH, WDATA_WIDTH, RDATA_WIDTH) apb3_monitor (
          .clk    (PCLK),     // Clock input
          .rst_n  (PRESETn),  // Active low reset
          .psel   (|(PSEL)),
          .penable(PENABLE),

...
module mgc_ahb_master_hdl #(
    ...
    mgc_ahb_signal_if pin_if
);  //pragma attribute mgc_ahb_master_hdl partition_module_xrtl
...
`else
  ...
  mgc_ahb #(NUM_MASTERS, NUM_MASTER_BITS, NUM_SLAVES, ADDRESS_WIDTH, WDATA_WIDTH, RDATA_WIDTH) vip_if (
      pin_if.HCLK,
      pin_if.HRESETn
  );
  ...
  initial begin
    ...
    uvm_pkg::uvm_config_db #(virtual mgc_ahb #(NUM_MASTERS, NUM_MASTER_BITS, NUM_SLAVES, ADDRESS_WIDTH, WDATA_WIDTH, RDATA_WIDTH))::
       set(
        null, VIP_IF_UVM_CONTEXT, VIP_IF_UVM_NAME, vip_if);
  end

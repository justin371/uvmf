...
module ahb_master (
  ...
  always @(posedge HCLK) begin
    if (HRESETn == 1'b1) begin
      if (HREADY == 1'b1) count <= count + 1;
      if (count == 10) begin  // do single transfers
        idle_off  <= 1'b1;
        sngl_tnfr <= 1'b1;
      end
      if (count == 20) begin  // do an idle cycle before burst
        idle_off  <= 1'b0;
        sngl_tnfr <= 1'b0;
      end
      if (count == 22) begin  // do burst transfers
        idle_off   <= 1'b1;
        burst_tnfr <= 1'b1;
        if (HREADY == 1'b1) HWRITE = ~HWRITE;
      end
      if (count == 30) count <= 0;
    end
  end
  ...
  always @(posedge HCLK) begin
    if (HRESETn == 1'b1 && idle_off == 1'b0) begin
    ...
    else if ((HRESETn == 1'b1) && (burst_tnfr == 1'b1) && (HGRANT == 1'b1)) begin
      if (HREADY == 1'b1) begin
        if (incr4 < 2'b11) incr4 <= incr4 + 1;
        if (sequential == 1'b0) begin
          HTRANS     <= 2'b10;
          HSIZE      <= 2'b0;
          HBURST     <= 3'b011;
          HPROT      <= 4'b0;
    ...
    end else if (HRESETn == 1'b0) begin
      HTRANS     <= 2'b0;
      HSIZE      <= 2'b0;
      HBURST     <= 3'b0;
      HPROT      <= 4'b0;
    ...
    if (((incr4 == 2'b11) || (HGRANT == 1'b0)) && (HREADY == 1'b1)) begin
      burst_tnfr <= 1'b0;
      idle_off   <= 1'b0;
      sequential <= 1'b0;
      incr4      <= 2'b00;
    ...
    if (cntr_phase == 1'b1 && HREADY == 1'b1 && HWRITE == 1'b1) begin

...
module ahb_slave (
  ...
  always @(posedge HCLK) begin
    if (HRESETn == 1'b1) begin
      if (HSEL && HREADYi) begin
        ...
        if (HTRANS == 2'b00) count = 0;
        if (HTRANS == 2'b11) begin
          if (count == 5) begin
            HREADYo <= 1'b0;
            wait_state_1 <= 1'b1;
          end else HREADYo <= 1'b1;
  ...
  always @(posedge HCLK) begin
    if (HRESETn == 1'b1) begin
      if (temp_write == 1'b1) begin
        for (i = 0; i < bytes_per_transfer; i = i + 1) begin
          data_memory[(temp_addr[11:0] + i)]= HWDATA[((i + (temp_addr[11:0]%(AHB_WDATA_WIDTH/8)))*8)+7 -:8];
`ifdef SLAVE_DEBUG_MEMORY
          $display("%d Writing to data_memory[%h] = %h", $time, temp_addr[11:0] + i,
                   data_memory[(temp_addr[11:0]+i)]);
`endif
        end
      end

      if ((HWRITE == 1'b0) && (HSEL && HREADYi) && (HTRANS[1] == 1)) begin
        temp_addr = HADDR % (1024 * (AHB_SLAVE_INDEX + 1));
        rdata_out = 0;
        for (i = 0; i < (1 << HSIZE); i = i + 1) begin
          rdata_out[((i + (temp_addr[11:0]%(AHB_WDATA_WIDTH/8)))*8)+7 -:8]= data_memory[(temp_addr[11:0] + i)];
`ifdef SLAVE_DEBUG_MEMORY
          $display("%d Reading from data_memory[%h] = %h", $time, temp_addr[11:0] + i,
                   data_memory[temp_addr[11:0]+i]);
`endif
        end
        HRDATA = rdata_out;
  ...
  always @(posedge HCLK) begin
    if (HRESETn == 1'b1) begin
      if (wait_state_1 == 1'b1) begin
        wait_state_1 <= 1'b0;
        wait_state_2 <= 1'b1;
      end
      if (wait_state_2 == 1'b1) begin
        wait_state_2 <= 1'b0;
      end
    end
  end

...
module AXI_slave_v (
  ...
  always @(posedge ACLK or negedge ARESETn) begin
    if (!ARESETn) begin
      ...
      BVALID_reg  <= BVALID;
      BID_reg     <= BID;
      for (i = 0; i < 4; i = i + 1) begin : id_for_loop
        case (write_states[i])
          0: begin  // Wait for write address
            if ((AWVALID == 1) && (AWREADY == 1) && (AWID == i)) begin
              write_addr[i] <= AWADDR;
          ...
          1: begin  // Wait for write data
            if ((WVALID == 1) && (WREADY == 1) && (WID == i)) begin
              ...
              if (WLAST == 1) begin
                ...
                incr = (AWSIZE_reg_val == 3'b000) ? 3 : (AWSIZE_reg_val == 3'b001) ? 1 : 0;
                if (write_count_first[i] == 1) begin
                  if ((write_count_incr[i] + (write_addr[i] % 4)) >= incr) begin
                    write_count[i] <= write_count[i] + 1;
                    write_count_incr[i] <= 0;
                    write_count_first[i] <= 0;
                  end else write_count_incr[i] <= write_count_incr[i] + 1;
                end // if (write_count_first[i]==1)
                               else
                                 begin
                  if (write_count_incr[i] >= incr) begin
                    write_count[i] <= write_count[i] + 1;
                    write_count_incr[i] <= 0;
                  end else write_count_incr[i] <= write_count_incr[i] + 1;
          ...
          2: begin  // Write response state
            if ((BVALID == 1) && (BREADY == 1) && (BID == i)) begin
              write_states[i] <= 0;
            end
          end
        endcase  // write
        case (read_states[i])
          0: begin  // Wait for read address
            if ((ARVALID == 1) && (ARREADY == 1) && (ARID == i)) begin
              read_addr[i]    <= ARADDR;
              read_length[i]  <= ARLEN;
          ...
          1: begin  // Read data state
            if ((RVALID == 1) && (RREADY == 1) && (RID == i)) begin
              read_length_sent[i] <= read_length_sent[i] + 1;
              if (RLAST == 1) begin
                read_states[i] <= 0;
              end else begin
                incr_read = (ARSIZE_reg_val == 3'b000) ? 3 : (ARSIZE_reg_val == 3'b001) ? 1 : 0;
                if (read_count_first[i] == 1) begin
                  if ((read_count_incr[i] + (read_addr[i] % 4)) >= incr) begin
                    read_count[i] <= read_count[i] + 1;
                    read_count_incr[i] <= 0;
                    read_count_first[i] <= 0;
                  end else read_count_incr[i] <= read_count_incr[i] + 1;
                end // if (read_count_first[i]==1)
                               else
                                 begin
                  if (read_count_incr[i] >= incr) begin
                    read_count[i] <= read_count[i] + 1;
                    read_count_incr[i] <= 0;
                  end else read_count_incr[i] <= read_count_incr[i] + 1;
  ...
  always_comb begin : output_drivers
    ...
    if ((RREADY_reg==1) || (RVALID_reg==0)) // Previous one accepted or no pevious one
        begin
      id = RID_reg + 1;
      if ((id != RID_reg) && (read_states[id] != 1)) begin
        id = id + 1;
        if ((id != RID_reg) && (read_states[id] != 1)) begin
          id = id + 1;
          if ((id != RID_reg) && (read_states[id] != 1)) begin
    ...
    if ((BREADY_reg==1) || (BVALID_reg==0)) // Previous one accepted or no pevious one
        begin
      id = BID_reg + 1;
      if ((id != BID_reg) && (write_states[id] != 2)) begin
        id = id + 1;
        if ((id != BID_reg) && (write_states[id] != 2)) begin
          id = id + 1;
          if ((id != BID_reg) && (write_states[id] != 2)) begin

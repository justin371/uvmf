...
function void trigger_named_event(string event_name, input int unsigned ret_value = 0);
  ...
  if (!event_pool_h.exists(event_name))
    `uvm_warning("dpi_link_event", {
                 "Triggered event: \"",
                 event_name,
                 "\" before the hvl was waiting for it. Perhaps a misspelled event name?"
                 });
...
class dpi_link_event extends uvm_object;
  ...
  protected
  function new(string name = "", string event_name = "");
    super.new(name);
  ...
  static function dpi_link_event get_event_using_xrtl_path(string xrtl_path);
    if (!scope_exists(xrtl_path))
      `uvm_fatal("dpi_link_event", {
                 "Tried to get an xrtl path event with non existant xrtl path: ", xrtl_path})

...
class uvmf_in_order_scoreboard #(
  ...
  function void write_actual(input T t);
    ...
    if (scoreboard_enabled) begin : in_write_actual
      ...
      if (expected_results_q.size() == 0) begin : empty_q
      ...
      else begin : item_to_compare_against
        ...
        last_actual   = t;
        if (t.compare(expected_transaction)) begin : compare_pass
          match_count++;
          last_mismatched = 0;
  ...
  virtual task wait_for_scoreboard_drain();
    while (expected_results_q.size() != 0) begin : while_entries_remain
      @entry_received;
  ...
  virtual function void check_phase(uvm_phase phase);
    ...
    if (end_of_test_empty_check && (expected_results_q.size() != 0)) begin : entries_remain
      while ( (expected_results_q.size() != 0) && ( fifo_entry <  max_remaining_transaction_print ))
              begin : print_entries
        expected_transaction = expected_results_q.pop_front();
        `uvm_info("SCBD", $sformatf(
                  "Entry %d:%s", fifo_entry++, expected_transaction.convert2string()), UVM_MEDIUM)
      end : print_entries

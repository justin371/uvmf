...
class uvmf_in_order_race_scoreboard #(
  ...
  function void write_expected(input T t);
    ...
    if (scoreboard_enabled && enable_expected_port) begin : in_write_expected
      ...
      if (actual_results_q.size() == 0) begin : no_actual_entry_to_compare_against
  ...
  function void write_actual(input T t);
    ...
    if (scoreboard_enabled && enable_actual_port) begin : in_write_actual
      ...
      if (expected_results_q.size() == 0) begin : no_expected_entry_to_compare_against
  ...
  virtual function void check_phase(uvm_phase phase);
    ...
    // Check expected fifo
    if (end_of_test_empty_check && (expected_results_q.size() != 0)) begin : expected_entries_remain
      while ( (expected_results_q.size() != 0) && ( fifo_entry <  max_remaining_transaction_print ))
        ...
        `uvm_info("SCBD", $sformatf(
                  "Expected Entry %d:%s", fifo_entry++, expected_transaction.convert2string()),
                  UVM_MEDIUM)
      end : print_expected_entries
      `uvm_error("SCBD", "EXPECTED SCOREBOARD NOT EMPTY");
    end : expected_entries_remain
    // Check actual fifo
    if (end_of_test_empty_check && (actual_results_q.size() != 0)) begin : actual_entries_remain
      fifo_entry = 0;
      while ( (actual_results_q.size() != 0) && ( fifo_entry <  max_remaining_transaction_print ))
        ...
        `uvm_info("SCBD", $sformatf(
                  "Actual Entry %d:%s", fifo_entry++, expected_transaction.convert2string()),
                  UVM_MEDIUM)

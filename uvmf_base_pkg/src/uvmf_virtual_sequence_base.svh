...
class uvmf_virtual_sequence_base #(
  ...
  `uvm_object_param_utils(uvmf_virtual_sequence_base#(CONFIG_T, BASE_T, ENV_T))
  ...
  virtual task pre_start();
    ...
    if (m_sequencer == null) begin
      `uvm_fatal("VSQR", {get_full_name(), " must be started on an environment virtual sequencer"})
      return;
    end
    if (!$cast(vsqr, m_sequencer)) begin
      `uvm_fatal("VSQR", {get_full_name(), " was started on an incompatible sequencer"})
      return;
    end
    configuration = vsqr.get_config();
    if (configuration == null) begin
      `uvm_fatal("VSQR", {get_full_name(), " virtual sequencer has no environment configuration"})
    end
    if (!$cast(env, vsqr.get_env())) begin
      `uvm_fatal("VSQR", {get_full_name(), " virtual sequencer has no compatible environment"})
    end
  endtask : pre_start

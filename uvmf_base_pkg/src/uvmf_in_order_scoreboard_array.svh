...
class uvmf_in_order_scoreboard_array #(
  ...
  virtual function void flush_scoreboard();
    foreach (expected_results_q[i]) expected_results_q[i].delete();
  endfunction
  ...
  virtual function void remove_entry(int unsigned key = 0);
    ...
    if (key >= ARRAY_DEPTH) begin : key_check
      `uvm_error("SCBD", $sformatf("Invalid key %d out of valid range between 0 and %d", key,
                                   ARRAY_DEPTH))
    end : key_check
    else begin : remove_entry
  ...
  function void write_actual(input T t);
    ...
    if (scoreboard_enabled) begin : in_write_actual
      ...
      if (expected_results_q[t.get_key()].size() == 0) begin : no_item_exists_in_selected_q
        ...
        `uvm_error("SCBD", $sformatf("NO PREDICTED ENTRY TO COMPARE AGAINST:%s",
                                     t.convert2string()))
      end : no_item_exists_in_selected_q
      else begin : item_exists_in_selected_q
        ...
        last_actual   = t;
        if (t.compare(expected_transaction)) begin : compare_passed
          match_count++;
  ...
  virtual function void check_phase(uvm_phase phase);
    ...
    if (end_of_test_empty_check) begin : end_of_test_empty_check
      foreach (expected_results_q[i]) begin : foreach_expected_results_q
        if (expected_results_q[i].size() != 0) begin : entries_remain
          while ( (expected_results_q[i].size() != 0) && ( fifo_entry <  max_remaining_transaction_print ))
            ...
            `uvm_info(
                "SCBD", $sformatf(
                "Channel %d Entry %d:%s", i, fifo_entry++, expected_transaction.convert2string()),
                UVM_MEDIUM)
          end : print_entries

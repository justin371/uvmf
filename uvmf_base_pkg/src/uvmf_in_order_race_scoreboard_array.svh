...
class uvmf_in_order_race_scoreboard_array #(
    ...
    int ARRAY_DEPTH = 1,
    type BASE_T = uvmf_scoreboard_base#(T)
) extends BASE_T;
  ...
  virtual function void remove_entry(int unsigned key = 0);
    ...
    if (real_key >= ARRAY_DEPTH) begin : key_check
      `uvm_error("SCBD", $sformatf("Invalid key %d out of valid range between 0 and %d", real_key,
                                   ARRAY_DEPTH))
    end : key_check
    else if (key[31] == 0) begin : remove_from_expected
      flushed_transaction = expected_results_q[real_key].pop_front();
    end : remove_from_expected
    else begin : remove_from_actual
  ...
  virtual function void write_expected(input T t);
    ...
    if (scoreboard_enabled && enable_expected_port) begin : in_write_expected
      ...
      if (actual_results_q[real_key].size() == 0) begin : no_actual_entry_to_compare_against
        expected_results_q[real_key].push_back(t);
      end : no_actual_entry_to_compare_against
      else begin : compare_against_actual
  ...
  virtual function void write_actual(input T t);
    ...
    if (scoreboard_enabled && enable_actual_port) begin : in_write_actual
      ...
      else begin : compare_against_expected
  ...
  function void compare_entries(T expected, T actual);
    ...
    if (actual.compare(expected)) begin : compare_passed
      match_count++;
  ...
  virtual task wait_for_scoreboard_drain();
    ...
    foreach (expected_results_q[i]) if (expected_results_q[i].size() != 0) entries_remaining |= 1;
  ...
  virtual function void check_phase(uvm_phase phase);
    ...
    if (end_of_test_empty_check) begin : end_of_test_empty_check
      foreach (expected_results_q[i]) begin : foreach_expected_results_q
        // Check expected fifo
        if (expected_results_q[i].size() != 0) begin : expected_entries_remain
          while ( (expected_results_q[i].size() != 0) && ( fifo_entry <  max_remaining_transaction_print ))
            ...
            `uvm_info("SCBD", $sformatf(
                      "Channel %d Expected Entry %d:%s",
                      i,
                      fifo_entry++,
                      expected_transaction.convert2string()
                      ), UVM_MEDIUM)
          end : print_expected_entries
          `uvm_error("SCBD", "EXPECTED SCOREBOARD NOT EMPTY");
        end : expected_entries_remain
        // Check actual fifo
        if (actual_results_q[i].size() != 0) begin : actual_entries_remain
          fifo_entry = 0;
